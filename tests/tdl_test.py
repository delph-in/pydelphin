...
def test_AVM():
    ...
    a = AVM(
        [("ATTR1", Conjunction([AVM([("ATTR2.ATTR3", Conjunction([String("b")]))])]))]
    )
    assert len(a.features()) == 1
    ...
    a = AVM(
        [
            (
                "ATTR1",
                Conjunction(
                    [TypeIdentifier("b"), AVM([("ATTR2", TypeIdentifier("c"))])]
                ),
            )
        ]
    )
...
class TestConjunction:
    ...
    def test__getitem__(self):
        ...
        a.add(AVM({"ATTR": TypeIdentifier("val2")}))  # two AVMs
        assert a["ATTR"] == Conjunction([TypeIdentifier("val"), TypeIdentifier("val2")])
    ...
    def test__setitem__(self):
        ...
        assert a["ATTR"] == Conjunction(
            [TypeIdentifier("val2"), TypeIdentifier("val3")]
        )
...
def test_TypeDefinition():
    ...
    t = TypeDefinition(
        "t", Conjunction([AVM([("ATTR", String("s"))]), TypeIdentifier("a")])
    )
    ...
    t = TypeDefinition("t", TypeIdentifier("a", docstring="a doc"), docstring="doc")
    assert t.docstring == "doc"
...
def test_format_AVM():
    ...
    assert (
        tdl.format(AVM([("ATTR", Conjunction([TypeIdentifier("a")]))])) == "[ ATTR a ]"
    )
    assert tdl.format(
        AVM(
            [
                ("ATTR1", Conjunction([TypeIdentifier("a")])),
    ...
    assert tdl.format(
        AVM(
            [
                (
                    "ATTR1",
                    Conjunction([AVM([("ATTR2", Conjunction([TypeIdentifier("a")]))])]),
                )
            ]
        )
    ) == ("[ ATTR1 [ ATTR2 a ] ]")
...
def test_format_lists():
    ...
    assert tdl.format(ConsList([String("a")], end=Coreference("b"))) == '< "a" . #b >'
...
def test_format_docstring_terms():
    assert tdl.format(TypeIdentifier("a", docstring="doc")) == '"""\ndoc\n"""\na'
    ...
    assert (
        tdl.format(TypeIdentifier("a", docstring='"one" ""two"" """three""" """"""""'))
        == '"""\n"one" ""two"" ""\\"three""\\" ""\\"""\\"""\n"""\na'
    )
...
def test_format_Conjunction():
    ...
    assert (
        tdl.format(Conjunction([TypeIdentifier("a"), TypeIdentifier("b")])) == "a & b"
    )
...
def test_format_typedefs():
    ...
    t2 = TypeDefinition(
        ...
        Conjunction(
            [
                TypeIdentifier("a"),
                AVM([("ATTR", TypeIdentifier("b"))], docstring="a doc"),
            ]
        ),
        docstring="t doc",
    )
...
def test_format_environments():
    ...
    e = TypeEnvironment(
        entries=[
            TypeDefinition(
                "id",
                Conjunction(
                    [TypeIdentifier("a"), AVM([("ATTR", TypeIdentifier("b"))])]
                ),
            )
        ]
    )
    ...
    e = TypeEnvironment(
        entries=[
            ...
            InstanceEnvironment(status="lex-rule", entries=[FileInclude("lrules.tdl")]),
...
def test_format_blockcomment():
    assert tdl.format(BlockComment(" a block comment ")) == "#| a block comment |#"
    assert tdl.format(BlockComment("\n  one\n  two\n")) == "#|\n  one\n  two\n|#"
