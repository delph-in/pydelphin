...
def it_rains_heavily_dmrs():
    d = DMRS(
        ...
        10,
        nodes=[
            Node(10, "_rain_v_1", type="e", properties={"TENSE": "past"}),
...
def abrams_barked_dmrs():
    d = DMRS(
        ...
        nodes=[
            Node(10, "udef_q"),
            Node(20, "named", type="x", carg="Abrams", lnk=Lnk.charspan(0, 6)),
            Node(
                30,
                "_bark_v_1",
                type="e",
        ...
        links=[Link(10, 20, "RSTR", "H"), Link(30, 20, "ARG1", "NEQ")],
...
def test_encode(it_rains_heavily_dmrs, abrams_barked_dmrs):
    ...
    assert simpledmrs.encode(it_rains_heavily_dmrs, properties=False, indent=True) == (
        "dmrs {\n"
        "  [top=20 index=10]\n"
        "  10 [_rain_v_1 e];\n"
        "  20 [_heavy_a_1 e];\n"
...
def test_decode_no_index_issue_334():
    ...
    d = simpledmrs.decode("dmrs { [top=10] 10 [_rain_v_1<3:9> e TENSE=past];}")
    assert d.index is None

    d = simpledmrs.decode("dmrs { 10 [_rain_v_1<3:9> e TENSE=past];}")
