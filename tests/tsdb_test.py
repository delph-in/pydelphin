...
def test_read_schema(empty_testsuite):
    ...
    f = tsdb.Field
    assert r == {
        "item": [f("i-id", ":integer", (":key",)), f("i-input", ":string")],
        "fold": [f("fold-id", ":integer", (":key",))],
        "run": [f("run-id", ":integer", (":key",))],
        "parse": [
            f("parse-id", ":integer", (":key",)),
...
def test_write_schema(empty_testsuite, tmp_path):
    ...
    r = OrderedDict(
        [
            ("item", [f("i-id", ":integer", (":key",)), f("i-input", ":string")]),
            ("fold", [f("fold-id", ":integer", (":key",))]),
            ("run", [f("run-id", ":integer", (":key",), "unique test run identifier")]),
            (
                "parse",
                [
                    f("parse-id", ":integer", (":key",), "unique parse identifier"),
                    f("run-id", ":integer", (":key",), "test run for this parse"),
                    f("i-id", ":integer", (":key",), "item parsed"),
                ],
            ),
            (
                "result",
                [
                    f("parse-id", ":integer", (":key",), "parse for this result"),
                    f("result-id", ":integer", (), "result identifier"),
                    f("mrs", ":string", (), "MRS for this reading"),
                ],
            ),
        ]
    )
...
def test_make_record(empty_testsuite):
    ...
    assert tsdb.make_record({"i-input": "one", "i-id": 100}, r["item"]) == (100, "one")
    assert tsdb.make_record({"i-id": 100}, r["item"]) == (100, None)
    assert tsdb.make_record(
        {"i-id": 100, "mrs": "[RELS: < > HCONS: < >]"}, r["item"]
    ) == (100, None)
...
def test_cast():
    ...
    assert tsdb.cast(":date", "01-dec-02 (15:31:01)") == datetime(
        2002, 12, 1, 15, 31, 1
    )
    assert tsdb.cast(":date", "2008-10-12 10:51") == datetime(2008, 10, 12, 10, 51)
...
def test_write_database(tmp_path, mini_testsuite, empty_alt_testsuite):
    ...
    assert tmp_ts.joinpath("parse").read_text() == ("10@10@1\n20@20@0\n30@30@1\n")
...
def test_bad_date_issue_279(tmp_path, empty_alt_testsuite):
    ...
    tsdb.write(tmp_ts, "item", [(0, "The cat meows.", datetime(1999, 9, 8))], fields)
    db = tsdb.Database(tmp_ts)
    assert list(db["item"]) == [("0", "The cat meows.", "8-sep-1999")]
    tsdb.write(tmp_ts, "item", [(0, "The cat meows.", "September 8, 1999")], fields)
