...
def test_type_map_multi_rule_no_semi():
    ...
    assert vpm.load(S("event >> e\nevent << u")).apply("e2", {}, reverse=True) == (
        "e2",
        {},
    )
    assert vpm.load(S("event >> e\nevent << u")).apply("u2", {}, reverse=True) == (
        "u2",
        {},
    )
...
def test_prop_map_no_semi():
    ...
    assert v.apply("e2", {"TENSE": "pres"}, reverse=True) == (
        "e2",
        {"E.TENSE": "present"},
    )
    ...
    assert v.apply("event2", {"PNG.PN": "1sg"}) == (
        "event2",
        {"PERS": "1", "NUM": "sg"},
    )
    assert v.apply("event2", {"PNG.PN": "1pl"}) == (
        "event2",
        {"PERS": "1", "NUM": "pl"},
    )
    assert v.apply("event2", {"PNG.PN": "1per"}) == ("event2", {"PERS": "1"})
    assert v.apply("event2", {"PNG.PN": "4du"}) == ("event2", {})
    assert v.apply("e2", {"PERS": "1", "NUM": "sg"}, reverse=True) == (
        "e2",
        {"PNG.PN": "1sg"},
    )
    assert v.apply("e2", {"PERS": "1", "NUM": "pl"}, reverse=True) == (
        "e2",
        {"PNG.PN": "1pl"},
    )
    assert v.apply("e2", {"PERS": "1"}, reverse=True) == ("e2", {"PNG.PN": "1per"})
    assert v.apply("e2", {"PERS": "1", "NUM": "du"}, reverse=True) == (
        "e2",
        {"PNG.PN": "1per"},
    )
    ...
    assert v.apply(
        "event2",
        {"E.ASPECT.SOON": "bool", "E.ASPECT.EVER": "bool", "E.ASPECT.ALREADY": "+"},
    ) == ("event2", {"TENSE": "past"})
    assert v.apply(
        "event2",
        {"E.ASPECT.SOON": "bool", "E.ASPECT.EVER": "+", "E.ASPECT.ALREADY": "bool"},
    ) == ("event2", {"TENSE": "nonpresent"})
    assert v.apply(
        "event2",
        {"E.ASPECT.SOON": "+", "E.ASPECT.EVER": "bool", "E.ASPECT.ALREADY": "bool"},
    ) == ("event2", {"TENSE": "fut"})
    assert v.apply("e2", {"TENSE": "past"}, reverse=True) == (
        "e2",
        {"E.ASPECT.SOON": "bool", "E.ASPECT.EVER": "bool", "E.ASPECT.ALREADY": "+"},
    )
    assert v.apply("e2", {"TENSE": "nonpresent"}, reverse=True) == (
        "e2",
        {"E.ASPECT.SOON": "bool", "E.ASPECT.EVER": "+", "E.ASPECT.ALREADY": "bool"},
    )
    assert v.apply("e2", {"TENSE": "fut"}, reverse=True) == (
        "e2",
        {"E.ASPECT.SOON": "+", "E.ASPECT.EVER": "bool", "E.ASPECT.ALREADY": "bool"},
    )
