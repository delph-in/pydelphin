...
def test_basic_external_group_active():
    _r = r.from_string(
        ">a",
        modules={"a": r.from_string(r"!a	b"), "b": r.from_string(r"!b	c")},
        active=["a"],
    )
    x = _r.apply("baba")
    assert x.string == "bbbb"
...
def test_indirect_external_group_active():
    ...
    assert (
        r.from_string(">a", modules={"a": a}, active=("a", "b")).apply("baba").string
        == "bbbb"
    )
...
def test_tokenizer_in_top_module_issue_308():
    ...
    assert (
        len(
            r.from_string(">a", modules={"a": r.from_string(r":[b]")})
            .tokenize("aba")
            .tokens
        )
        == 1
