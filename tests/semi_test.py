...
def test_include(tmp_path):
    ...
    # redefinition
    a.write_text("variables:\n  i.\ninclude: b.smi\n")
    b.write_text("variables:\n  i.\n")
    with pytest.warns(semi.SemIWarning):
        semi.load(str(a))
...
def test_consistency():
    ...
    with pytest.raises(hierarchy.HierarchyError):
        semi.SemI(variables={"u": {"parents": []}, "i": {"parents": ["u", "i"]}})
    # undeclared variable
    ...
    with pytest.raises(semi.SemIError):
        semi.SemI(
            variables={"u": {"parents": []}, "i": {"parents": ["u"]}},
            predicates={
                "_predicate_n_1": {
                    "parents": [],
    ...
    with pytest.raises(semi.SemIError):
        semi.SemI(
            variables={
                "u": {"parents": []},
                "i": {"parents": ["u"], "properties": [["IND", "bool"]]},
            },
            roles={"ARG0": {"value": "i"}},
...
def test_to_dict(tmp_path):
    ...
    assert d["predicates"]["_predominant_a_1"] == {
        "synopses": [
            {"roles": [{"name": "ARG0", "value": "e"}, {"name": "ARG1", "value": "e"}]},
            {"roles": [{"name": "ARG0", "value": "e"}, {"name": "ARG1", "value": "p"}]},
        ]
    }
