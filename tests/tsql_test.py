...
def test_parse_select_where():
    ...
    assert tsql.inspect_query("select i-input where i-date < 2018-01-15")[
        "condition"
    ] == ("<", ("i-date", datetime(2018, 1, 15)))

    assert tsql.inspect_query("select i-input where i-date > 15-jan-2018(15:00:00)")[
        "condition"
    ] == (">", ("i-date", datetime(2018, 1, 15, 15, 0, 0)))

    assert tsql.inspect_query('select i-input where i-input ~ "Abrams"')[
        "condition"
    ] == ("~", ("i-input", "Abrams"))

    assert tsql.inspect_query("select i-input where i-input !~ 'Browne'")[
        "condition"
    ] == ("!~", ("i-input", "Browne"))

    assert tsql.inspect_query("select i-input where i-wf = 2 & i-input ~ '[Dd]og'")[
        "condition"
    ] == ("and", [("==", ("i-wf", 2)), ("~", ("i-input", "[Dd]og"))])

    assert tsql.inspect_query("select i-input where i-id = 10 | i-id = 20 & i-wf = 2")[
        "condition"
    ] == (
        "or",
        [("==", ("i-id", 10)), ("and", [("==", ("i-id", 20)), ("==", ("i-wf", 2))])],
    )

    assert tsql.inspect_query(
        "select i-input where (i-id = 10 | i-id = 20) & !i-wf = 2"
    )["condition"] == (
        "and",
        [
            ("or", [("==", ("i-id", 10)), ("==", ("i-id", 20))]),
