...
def dump(ds, destination, properties=False, lnk=True, indent=False, encoding="utf-8"):
    """
...
def dumps(ds, properties=False, lnk=True, indent=False):
    ...
    graphs = [to_graph(to_triples(d, properties=properties, lnk=lnk)) for d in ds]
    try:
...
def to_triples(d, properties=True, lnk=True):
    ...
    if not complete:
        logger.warning("disconnected graph cannot be completely encoded: %r", d)
    return triples
