...
from .config import (ANCHOR_SORT, CVARSORT, CVARG, QEQ, VARIABLE_ARG,
                     LTOP_NODEID, EQ_POST, NEQ_POST, HEQ_POST, H_POST)
...
class Xmrs(LnkMixin):
    ...
    def __init__(self, hook=None, nodes=None, args=None,
        ...
        for node in nodes:
            if node.is_quantifier(): continue
            nid = node.nodeid
    ...
    def label_set_heads(self, label):
