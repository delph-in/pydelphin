...
parser.add_argument(
    ...
    nargs="?",
    help=(
        "file with representations to convert or testsuite directory "
        "from which result.mrs will be selected; if not given, "
        "<stdin> is read as though it were a file"
    ),
)
parser.add_argument(
    "--list", action="store_true", help="list the available codecs and capabilities"
)
parser.add_argument(
    "-f",
    "--from",
...
parser.add_argument(
    "--no-properties", action="store_true", help="suppress morphosemantic properties"
)
parser.add_argument(
    "--no-lnk",
    action="store_true",
...
parser.add_argument(
    ...
    default="result.mrs",
    help=(
        "TSQL query for selecting MRS data when PATH points to "
