...
def call_compare(args):
    ...
    if args.verbosity > 0:
        template += "\t{input}"
    for result in compare(args.TESTSUITE, args.GOLD, select=args.select):
        print(template.format(**result))


parser.set_defaults(func=call_compare)
parser.add_argument("TESTSUITE", help="path to the current test-suite directory")
parser.add_argument("GOLD", help="path to the gold test-suite directory")
parser.add_argument(
    "--select",
    metavar="QUERY",
    default="item.i-id item.i-input result.mrs",
    help=(
        "TSQL query for selecting (id, input, mrs) triples from "
