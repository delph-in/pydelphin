...
class Derivation(object):
    ...
    ace_pattern = " ".join(("(?P<EDGE_ID>\d+)",
                            ...
                            "(?P<TOKEN>({0}[\w{1}]+{0}|nil))",
                            "(?P<CHART_ID>\d+)",
                            "(?P<RULE_NAME>[\w{1}]+)",)).format("[\'\"]?", re.escape("-+=/:.!?$<>@#%^&*"))

    def __init__(self, definition, data_package=None, children=None):
        """
        Two constructors:
            Derivation(str:definition):
                Construct a Derivation object from a bracketed tree structure.
                ex. Derivation("(S (NP (N I)) (VP (V fell)))")
    ...
    # HTML Methods
    def output_HTML(self, title_text=True):
        """
        ...
        values = {
            ...
            "CHILDREN": "".join(child.output_HTML(title_text=title_text) for child in self.children)
        }
        # Return result
