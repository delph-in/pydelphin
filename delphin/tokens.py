...
class YYToken(_YYToken):
    ...
    def __str__(self):
        ...
        parts.extend([str(self.ipos), " ".join(map('"{}"'.format, self.lrules))])
        if self.pos:
    ...
    def to_dict(self):
        ...
        """
        d = {"id": self.id, "start": self.start, "end": self.end, "form": self.form}
        if self.lnk:
...
class YYTokenLattice:
    ...
    def __eq__(self, other):
        ...
        if len(self.tokens) == len(other.tokens) and all(
            t1 == t2 for t1, t2 in zip(self.tokens, other.tokens, strict=True)
        ):
            return True
        return False
