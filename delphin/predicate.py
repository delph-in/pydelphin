...
Semantic predicates are atomic symbols representing semantic entities
or constructions. For example, in the `English Resource Grammar
<http://www.delph-in.net/erg/>`_, `_mouse_n_1` is the predicate for
the word *mouse*, but it is underspecified for lexical semantics---it
could be an animal, a computer's pointing device, or something
else. Another example from the ERG is `compound`, which is used to
link two compounded nouns, such as for *mouse pad*.
...
the semantic representation of surface (i.e., lexical) tokens, such as
the `_mouse_n_1` example above. In form, they must always begin with a
...
sense, but in PyDelphin (as of `v1.0.0`_) predicates are always simple
strings. However, this module has functions for composing and
decomposing predicates from/to their components (the :func:`create`
and :func:`split` functions, respectively). In addition, there are
functions to normalize (:func:`normalize`) and validate
