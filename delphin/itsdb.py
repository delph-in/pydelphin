    ...
    * :meth:`collect` -- yield :class:`~delphin.interface.Response`
    ...
    def __init__(self, source: tsdb.Database | None = None):
        ...
        if source:
            pairs = typing_cast(
                list[tuple[int, int]],
                source.select_from("parse", ("parse-id", "i-id"), cast=True),
            )
    ...
    def _map_result(self, result: interface.Result, parse_id: int) -> tsdb.ColumnMap:
    ...
    def _map_edge(self, edge: tsdb.ColumnMap, parse_id: int) -> tsdb.ColumnMap:
...
class Row(tsdb.Record):
    ...
    def __init__(
        ...
        self.fields = fields
        self.data = tuple(
            tsdb.format(f.datatype, val) for f, val in zip(fields, data, strict=True)
        )
        self._field_index = field_index

    def __repr__(self) -> str:
        return "<{} object ({}) at {}>".format(
            type(self).__name__, ", ".join(map(repr, self)), id(self)
        )
...
class Table(tsdb.Relation):
    ...
    def _in_transaction(self) -> bool:
        ...
        return (
            num_recs > self._persistent_count
    ...
    def __setitem__(self, index, value):  # noqa: F811
        ...
        for i, row in enumerate(values):
            values[i] = Row(self.fields, row, field_index=self._field_index)
        self._rows[index] = values
        self._volatile_index = min(
            self._volatile_index, min(index.indices(len(self._rows))[:2])
        )
    ...
    def extend(self, rows: Iterable[tsdb.Record]) -> None:
        ...
        for row in rows:
            if not isinstance(row, Row):
                row = Row(self.fields, row, field_index=self._field_index)
            self._rows.append(row)

    def update(self, index: int, data: tsdb.ColumnMap) -> None:
        """
        ...
        self[index] = Row(self.fields, values, field_index=self._field_index)
    ...
    def _enum_rows(
        ...
        for i, row in enumerate(self._rows):
            ...
            if row is None:
                if line is not None:
                    row = Row(fields, tsdb.split(line), field_index=field_index)
                else:
                    continue
            yield (i, row)
...
class TestSuite(tsdb.Database):
    ...
    def __init__(
        ...
        if not path.joinpath(tsdb.SCHEMA_FILENAME).is_file():
            if schema is None:
                raise ITSDBError("*schema* argument is required for new test suites")
    ...
    def in_transaction(self) -> bool:
        ...
        data = self._data
        return any(data[name]._in_transaction for name in self.schema if name in data)
    ...
    def __getitem__(self, name: str) -> Table:
        ...
        if name not in self._data:
            self._data[name] = Table(self.path, name, self.schema[name], self.encoding)
        return self._data[name]

    def select_from(
        self, name: str, columns: Iterable[str] | None = None, cast: bool = True
    ):
        """
    ...
    def processed_items(
        self, fieldmapper: FieldMapper | None = None
    ...
    def process(
        ...
        if input_table not in self.schema or all(
            f.name != input_column for f in self.schema[input_table]
        ):
            raise ITSDBError(
        ...
        for row in source[input_table]:
            ...
            logger.info(
                "Processed item {:>16}  {:>8} results".format(
